  ...
  if (msec >= 20) {
    ...
    for (int b = 0; b < NUM_USERPOTS; b++) {
      ...
      if (((f + userPotPrevs[0][b] + userPotPrevs[1][b] + userPotPrevs[2][b]) / 4) != userPotSends[b])  {
        usbMIDI.sendControlChange(USERPOTS_MIDI[b], 127 - ((f + userPotPrevs[0][b] + userPotPrevs[1][b] + userPotPrevs[2][b]) / 4), channel);
